...
class InteractionAgent:
    ...
    def __init__(self, rules_provider: LocalJsonInteractionProvider | None = None, drug_service: DrugDataService | None = None, data_path: Path | None = None):
    ...
    def analyze(self, patient: Patient, new_medication: str, language: str = "tr") -> list[RiskFinding]:
        ...
        for index, item in enumerate(self.rules_provider.get_interaction_rules()):
            ...
            exists = (drug_a in new_aliases and drug_b in current_aliases) or (drug_b in new_aliases and drug_a in current_aliases)
            if not exists:
                continue

            matched_rules.add(index)
            title = f"{item['drug_a']} - {item['drug_b']} interaction" if lang == "en" else f"{item['drug_a']} - {item['drug_b']} etkileşimi"
            description = localize_value(item.get("description"), lang, fallback="An interaction rule was found for this medication combination." if lang == "en" else "Bu ilaç kombinasyonu için etkileşim kuralı bulundu.")
            recommendation = localize_value(item.get("recommendation"), lang, fallback="Reassess the combination in clinical context." if lang == "en" else "Kombinasyonu klinik bağlamda yeniden değerlendirin.")
            findings.append(RiskFinding(
                title=title,
                severity=item.get("severity", "medium"),
                description=description,
                recommendation=recommendation,
                source=item.get("source", "Curated interaction database"),
