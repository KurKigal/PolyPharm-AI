...
RENAL_SENSITIVE_MEDICATIONS = {
    "metformin", "furosemide", "ibuprofen", "naproxen", "lisinopril", "enalapril", "digoxin",
}
HEPATIC_RISK_MEDICATIONS = {
    "atorvastatin", "simvastatin", "warfarin", "paracetamol", "acetaminophen", "methotrexate",
}
...
class LabRiskAgent:
    def analyze(self, patient: Patient, new_medication: str, language: str = "tr") -> list[RiskFinding]:
        lang = normalize_language(language)
        tx = TEXT[lang]
        new_med = _normalize(new_medication)
        findings: list[RiskFinding] = []
        findings.extend(self._renal_findings(patient.lab_values.egfr, new_med, tx))
        findings.extend(self._hepatic_findings(patient.lab_values.ast, patient.lab_values.alt, new_med, tx))
        findings.extend(self._polypharmacy_findings(patient, tx))
        return findings

    def _renal_findings(self, egfr: float, new_medication: str, tx: dict[str, str]) -> list[RiskFinding]:
        renal_sensitive = new_medication in RENAL_SENSITIVE_MEDICATIONS
        if egfr < 30:
            return [RiskFinding(
                title=tx["renal_severe_title"], severity="high" if renal_sensitive else "medium",
                description=tx["renal_severe_desc"].format(egfr=egfr), recommendation=tx["renal_severe_rec"],
                source="Rule-based clinical prototype logic", agent="LabRiskAgent",
            )]
        if egfr < 60:
            return [RiskFinding(
                title=tx["renal_mod_title"], severity="medium" if renal_sensitive else "low",
                description=tx["renal_mod_desc"].format(egfr=egfr), recommendation=tx["renal_mod_rec"],
                source="Rule-based clinical prototype logic", agent="LabRiskAgent",
            )]
        return []

    def _hepatic_findings(self, ast: float, alt: float, new_medication: str, tx: dict[str, str]) -> list[RiskFinding]:
        hepatic_sensitive = new_medication in HEPATIC_RISK_MEDICATIONS
        if ast > 120 or alt > 120:
            return [RiskFinding(
                title=tx["hepatic_marked_title"], severity="high" if hepatic_sensitive else "medium",
                description=tx["hepatic_marked_desc"].format(ast=ast, alt=alt), recommendation=tx["hepatic_marked_rec"],
                source="Rule-based clinical prototype logic", agent="LabRiskAgent",
            )]
        if ast > 80 or alt > 80:
            return [RiskFinding(
                title=tx["hepatic_title"], severity="medium",
                description=tx["hepatic_desc"].format(ast=ast, alt=alt), recommendation=tx["hepatic_rec"],
                source="Rule-based clinical prototype logic", agent="LabRiskAgent",
            )]
        return []

    def _polypharmacy_findings(self, patient: Patient, tx: dict[str, str]) -> list[RiskFinding]:
        count = len(patient.current_medications)
        if patient.age >= 65 and count >= 5:
            return [RiskFinding(
                title=tx["poly_elderly_title"], severity="medium",
                description=tx["poly_elderly_desc"].format(age=patient.age, count=count), recommendation=tx["poly_elderly_rec"],
                source="Rule-based clinical prototype logic", agent="LabRiskAgent",
            )]
        if count >= 8:
            return [RiskFinding(
                title=tx["poly_title"], severity="medium",
                description=tx["poly_desc"].format(count=count), recommendation=tx["poly_rec"],
                source="Rule-based clinical prototype logic", agent="LabRiskAgent",
            )]
        return []
