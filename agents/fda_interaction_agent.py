...
SEVERITY_WINDOW = 160
HIGH_RISK_MARKERS = (
    "contraindicated", "avoid", "should not be used", "do not use", "serious", "fatal",
    "life-threatening", "increased risk of bleeding",
)
...
class FdaLabelInteractionAgent:
    def __init__(self, openfda_client: OpenFdaClient, drug_service: DrugDataService | None = None, rules_provider: LocalJsonInteractionProvider | None = None):
    ...
    def _local_rule_pairs(self) -> set[frozenset[str]]:
        ...
        for rule in self.rules_provider.get_interaction_rules():
            a, b = _normalize(rule.get("drug_a", "")), _normalize(rule.get("drug_b", ""))
            if a and b:
    ...
    def _label_text(self, drug_name: str) -> str | None:
        ...
        return None if label is None else label.drug_interactions_full
    ...
    def _find_mention(self, text: str, aliases: list[str]) -> tuple[str, str, str] | None:
        for alias in aliases:
            ...
            severity = "high" if any(marker in window for marker in HIGH_RISK_MARKERS) else "medium"
            start, end = max(0, match.start() - EXCERPT_RADIUS), min(len(text), match.end() + EXCERPT_RADIUS)
    ...
    def analyze(self, patient: Patient, new_medication: str, language: str = "tr") -> list[RiskFinding]:
        ...
        def try_report(label_owner: str, current_medication: str, mentioned_name: str, mention: tuple[str, str, str]) -> None:
            ...
            if lang == "en":
                title = f"{new_medication} - {current_medication} interaction (FDA label)"
                description = f"The interaction section of the FDA label for {label_owner} contains a warning related to {mentioned_name}: “{excerpt}”"
                recommendation = "Assess the official label warning in patient context and review dose, monitoring, or therapeutic alternatives."
            else:
                title = f"{new_medication} - {current_medication} etkileşimi (FDA prospektüsü)"
                description = f"{label_owner} ilacının FDA prospektüsündeki etkileşim bölümünde {mentioned_name} ile ilgili uyarı bulundu: “{excerpt}”"
                recommendation = "Resmi prospektüsteki etkileşim uyarısı hasta özelinde değerlendirilmeli; doz, izlem veya alternatif tedavi gözden geçirilmelidir."
            findings.append(RiskFinding(title=title, severity=severity, description=description, recommendation=recommendation, source="openFDA drug label (drug_interactions)", agent="FdaLabelInteractionAgent"))

        new_text = self._label_text(new_medication)
        if new_text:
            for medication in current_meds:
                mention = self._find_mention(new_text, self._aliases(medication))
